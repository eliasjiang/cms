...
def home_index():
    ...
    return UtilHelper.renderView("admin/user/profile/index.html", {
...
def set_info():
    if UtilHelper.isGet():
        return UtilHelper.renderPopView("admin/user/profile/set_info.html", {"info": CurrentUserService.getInfo() })
    req = request.values
...
def home_news():
    ...
    return UtilHelper.renderView("admin/user/profile/news.html",{"list": list,"pages":pages,"sc":sc })
...
def bind_open_wechat():
    return UtilHelper.renderPopView("admin/user/profile/bind_open_wechat.html")
