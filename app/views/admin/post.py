...
def create_post():
    current_app.logger.debug("Enter function create_post!")
    post_form = PostForm()
...
def get_post_info(form):
    current_app.logger.debug("Enter function get_post_info!")
    post = Post()
    post.title = form.title.data
    post.slug = form.slug.data
    # todo get user info
    # post.authorid = User.query.get(form.authorid.data)
    # post.authorid = User.query.get(1)
    post.excerpt = form.excerpt.data
    post.content = form.content.data
    post.categoryid = form.categoryid.data
    post.status = strtobool(form.status.data)
    # todo get tag info
    # post.tag = form.title.data
    # current_app.logger.debug(jsonify(query_to_dict(post)))
