...
class PostResource:
    ...
    def query_posts():
        ...
        sql = 'SELECT t_post.id AS post_id, t_post.title AS post_title, t_post.slug AS post_slug, ' \
              ...
              'FROM t_post, t_category, t_user WHERE t_post.categoryid = t_category.id ' \
              'AND t_post.authorid = t_user.id ' \
              'ORDER BY t_post.publishtime DESC'
        try:
            posts = db.session.execute(sql)
            posts = list(posts)  # 转换为list，解决Posts为Cursor类型，不能使用len判断为空的问题
